...
const NodeDetailsPopup = ({
  ...
  try {
    ...
    if (!rawSchema) throw new Error("Could not find root schema in session storage");

    const rootSchema = JSON.parse(rawSchema);
    ...
    let subschema: any = rootSchema;
    for (const segment of path) {
      if (subschema && typeof subschema === "object" && segment in subschema) {
        subschema = subschema[segment];
      } else {
        subschema = undefined;
        break;
      }
    ...
    navigator.clipboard
      ...
      .catch((err) => {
        console.error("Failed to copy subschema: ", err);
      });
  } catch (err) {
    console.error("Error copying subschema:", err);
  }
};
